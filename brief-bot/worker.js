...
// Определение часового пояса по городу: стем названия (без окончания,
// чтобы ловить склонения: «из Казани») → смещение от UTC в часах.
// Напоминания шлём в рабочее окно по МЕСТНОМУ времени клиента.
const CITY_TZ = [
  // UTC+2
  ['калининград', 2],
  // UTC+3 — Москва и вся европейская часть (по умолчанию, можно не перечислять,
  // но явные записи полезны для уверенного матча)
  ['москв', 3], ['петербург', 3], ['питер', 3], ['спб', 3],
  ['казан', 3], ['нижн', 3], ['ростов', 3], ['краснодар', 3], ['сочи', 3],
  ['воронеж', 3], ['волгоград', 3], ['тула', 3], ['тверь', 3], ['рязан', 3],
  ['ярославл', 3], ['белгород', 3], ['липецк', 3], ['курск', 3], ['брянск', 3],
  ['иванов', 3], ['владимир', 3], ['калуг', 3], ['смоленск', 3], ['мурманск', 3],
  ['архангельск', 3], ['вологд', 3], ['петрозаводск', 3], ['псков', 3],
  ['севастопол', 3], ['симферопол', 3], ['крым', 3], ['ставропол', 3],
  ['махачкал', 3], ['грозн', 3], ['владикавказ', 3], ['нальчик', 3],
  ['пенз', 3], ['тамбов', 3], ['орел', 3], ['орёл', 3], ['кострром', 3],
  ['киров', 3], ['чебоксар', 3], ['йошкар', 3], ['саранск', 3], ['сыктывкар', 3],
  ['минск', 3], ['гомел', 3], ['брест', 3], ['витебск', 3], ['могилев', 3],
  // UTC+4
  ['самар', 4], ['тольятти', 4], ['саратов', 4], ['ульяновск', 4],
  ['ижевск', 4], ['астрахан', 4],
  // UTC+5
  ['екатеринбург', 5], ['челябинск', 5], ['перм', 5], ['уфа', 5], ['уфе', 5],
  ['уфы', 5], ['тюмен', 5], ['оренбург', 5], ['магнитогорск', 5],
  ['сургут', 5], ['нижневартовск', 5], ['курган', 5], ['ханты', 5],
  ['салехард', 5], ['астан', 5], ['алмат', 5], ['ташкент', 5], ['актобе', 5],
  ['караганд', 5], ['шымкент', 5],
  // UTC+6
  ['омск', 6], ['бишкек', 6],
  // UTC+7
  ['новосибирск', 7], ['красноярск', 7], ['барнаул', 7], ['томск', 7],
  ['кемеров', 7], ['новокузнецк', 7], ['абакан', 7], ['норильск', 7],
  ['горно-алтайск', 7], ['кызыл', 7],
  // UTC+8
  ['иркутск', 8], ['улан-удэ', 8], ['братск', 8], ['ангарск', 8],
  // UTC+9
  ['якутск', 9], ['чита', 9], ['чите', 9], ['читы', 9], ['благовещенск', 9],
  // UTC+10
  ['владивосток', 10], ['хабаровск', 10], ['уссурийск', 10], ['находк', 10],
  ['биробиджан', 10],
  // UTC+11
  ['магадан', 11], ['сахалин', 11], ['южно-сахалинск', 11],
  // UTC+12
  ['камчат', 12], ['петропавловск-камчатский', 12], ['анадыр', 12], ['чукот', 12],
];

// Возвращает смещение клиента от UTC в минутах; не распознали — Москва
function cityToOffsetMin(text) {
  const t = ' ' + String(text).toLowerCase().replace(/ё/g, 'е');
  for (const [stem, h] of CITY_TZ) {
    const idx = t.indexOf(stem);
    if (idx > 0 && !/[а-яa-z]/.test(t[idx - 1])) return h * 60;
  }
  return MSK_OFFSET_MIN;
}
...
const STEPS = [
  ...
  },
  {
    ...
    label: 'Город',
    kind: 'text',
    prompt: '📍 Из какого города вы обращаетесь?',
  },
  {
    key: 'E-mail',
    label: 'E-mail',
  ...
  async scheduled(event, env) {
    ...
    do {
      ...
      for (const { name } of page.keys) {
        ...
        // тихие часы по местному времени клиента (пояс — из вопроса про город)
        const localMin = (utcMin + (state.tz_offset_min ?? MSK_OFFSET_MIN)) % 1440;
        if (localMin < WORK_START_MIN || localMin >= WORK_END_MIN) continue;
...
async function onMessage(env, msg) {
  ...
  // по городу вычисляем часовой пояс — напоминания уйдут по местному времени
  if (step.key === 'Город') state.tz_offset_min = cityToOffsetMin(answer.text);
